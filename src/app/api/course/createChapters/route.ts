...
// above is the endpoint - run this when reached here
...
export async function POST(req: Request, res: Response) {
    try{
        ...
        console.log("reached route.ts")
        const body = await req.json()
        const {title, units} = chapterSchema.parse(body);
        ...
        const imageSearchTerm = await strict_output(
            "you are an AI capable of finding the most relevant image for a course",`Please provide a good image search term for the title of a course about ${title}. This search term will be fed into the unsplash API, so make sure it is a good search term that will return good search results`,
            {
